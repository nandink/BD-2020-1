...
if ($_POST){
    $buscar_user="SELECT * FROM personas WHERE IDpersona = ?";
    $sql_user=$bd->prepare($buscar_user);
    $sql_user->execute(array($_SESSION['ID']));
    $resultado=$sql_user->fetch();
    $autorcito = $resultado["nombre_usuario"];
    $nombre_album = $_POST["nombrealbum"];
    $buscararti="SELECT * FROM artistas WHERE IDpersona = ?";
    $sql_ar=$bd->prepare($buscararti);
    $sql_ar->execute(array($_SESSION['ID']));
    $resultado2=$sql_ar->fetch();
    $artid = $resultado2['IDartista'];
    $insert_al = 'INSERT INTO albumes (IDartista, Nombre_album, Autor_album) VALUES (?,?,?)';
    $agregara = $bd->prepare($insert_al);
    $agregara->execute(array($artid,$nombre_album,$autorcito));
    echo 'Album creado!';
}
?>
<!DOCTYPE html>
...
<div class="w3-main" style="margin-left:300px">
  ...
  <div class="w3-row-padding">
	...
	<div class="form-group input-group justify-content-center">
        ...
        <input name="nombrealbum" class="form-control" placeholder="Nombre de album" type="text">
    </div> <!-- form-group// -->
	<div class="form-group">
        <button type="submit" class="btn btn-primary btn-block"> Crear album </button>
    </div> <!-- form-group// -->     
      </div>
    </div>
  </div>
