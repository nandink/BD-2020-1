...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block"> 
    <a href="crearplaylists.php" onclick="w3_close()" class="w3-bar-item w3-button w3-padding"><i class="fa fa-plus fa-fw w3-margin-right"></i>CREAR PLAYLIST</a> 
    <a href="agregarplaylists.php" onclick="w3_close()" class="w3-bar-item w3-button w3-padding"><i class="fa fa-plus fa-fw w3-margin-right"></i>AGREGAR CANCION A PLAYLIST</a> 
    ...
    <a href="borrarcuentanormal.php" onclick="w3_close()" class="w3-bar-item w3-button w3-padding"><i class="fa fa-times fa-fw w3-margin-right"></i>ELIMINAR CUENTA</a>
...
<div class="w3-main" style="margin-left:300px">
  ...
  <br></br>
  <h3><b>Tus playlist</b></h3>
  <?php
        $sql0='SELECT * FROM playlist WHERE IDpersona= (?)';
        $sentencia0=$bd->prepare($sql0);
        $sentencia0->execute(array($_SESSION['ID']));
        $resultado0=$sentencia0->fetchAll();
        if(!$resultado0){
          echo "No has creado ninguna playlist :(";
        }
        foreach($resultado0 as $dato):
      ?>
    <div class="w3-third w3-container w3-margin-bottom">
      ...
      <div class="w3-container w3-white">
        ...
        <p>Seguidores: <?php echo $dato['cant_seguidore']?></p>
      </div>
    </div>
    <?php endforeach ?>
    <br></br>
  ...
  <br></br>
  <h3><b>Canciones que te gustan</b></h3>
  <?php
        $sql3='SELECT * FROM usuarios where IDpersona=(?)';
        $sentencia3=$bd->prepare($sql3);
        $sentencia3->execute(array($_SESSION["ID"]));
        $resultado3=$sentencia3->fetch();

        $sql4='SELECT * FROM gustan where IDusuario=(?)';
        $sentencia4=$bd->prepare($sql4);
        $sentencia4->execute(array($resultado3['IDusuario']));
        $resultado4=$sentencia4->fetchAll();
        if (!$resultado4){
          echo "No le has dado like a ninguna cancion :(";
        }

        foreach($resultado4 as $dato2):
          $sql5='SELECT * FROM canciones WHERE IDcancion= (?)';
          $sentencia5=$bd->prepare($sql5);
          $sentencia5->execute(array($dato2['IDcancion']));
    ...
    <div class="w3-third w3-container w3-margin-bottom">
      <img src="/w3images/mountains.jpg" alt="" style="width:100%" class="w3-hover-opacity">
      <div class="w3-container w3-white">
        <p><b><?php echo $data5['nombre_cancion']?> </b></p>
        <p>Artista: <?php echo $data5['autor']?></p>
        <p>Duracion: <?php echo $data5['Duracion']?> minutos</p>
        <p>Genero: <?php echo $data5['Genero']?></p>
      </div>
    </div>
    <?php endforeach ?>
    <?php endforeach ?>
